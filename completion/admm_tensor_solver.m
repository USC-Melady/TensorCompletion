...
for i = 1:m
    ...
    P(sub_idx{i,1}, sub_idx{i,2}, sub_idx{i,3}) = P(sub_idx{i,1}, sub_idx{i,2}, sub_idx{i,3}) +1;% extract subtensor
end
comp_sub = find(P>=1);% this is different from standard version
completable_ind = sub2ind(sz, comp_sub(:,1)', comp_sub(:,2)', comp_sub(:,3)')';
...
% solve Z: closed form solution for each i
    ...
    for i = 1:m
        for n = 1:N
           tmp(sub_idx{i,1}, sub_idx{i,2}, sub_idx{i,3}) = tmp(sub_idx{i,1}, sub_idx{i,2}, sub_idx{i,3}) + rho * X{i,n} + Y{i,n};      
        end
    end
    ...
    Z(ind_2) = tmp(ind_2)./(N*rho * P(ind_2));
    
% update Y
    for i = 1:m
        for n = 1:n
        Y{i,n} = Y{i,n}  + rho * (X{i,n} - Z(sub_idx{i,1}, sub_idx{i,2}, sub_idx{i,3} ));
        end
    end
    % objective funciton 
